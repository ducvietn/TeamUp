...
import { FiUpload, FiCheck, FiX, FiClock, FiUser, FiFile } from 'react-icons/fi';
...
const TaskDetail = () => {
  ...
  const handleProgressChange = async (newProgress) => {
    setUpdating(true);
    try {
      const response = await taskAPI.updateProgress(taskId, newProgress);
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              />
              <span className="text-lg font-bold text-primary-600 w-16 text-right">
                {task.progress}%
              </span>
            </div>
          ...
          {submissions.length > 0 && (
            <div className="card">
              <h3 className="font-semibold text-gray-900 mb-4">Lịch sử nộp bài</h3>
              <div className="space-y-4">
                {submissions.map((sub) => (
                  <div key={sub._id} className="border-b border-gray-100 pb-4 last:border-0 last:pb-0">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`badge ${
                          ...
                          {sub.status === 'approved' ? 'Đã duyệt' :
                           sub.status === 'rejected' ? 'Từ chối' : 'Chờ duyệt'}
                        </span>
                        {sub.isLate && (
                          <span className="badge bg-red-100 text-red-700">Trễ hạn</span>
                        )}
                      </div>
